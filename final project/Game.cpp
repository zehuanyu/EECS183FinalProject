...
bool Game::isValidPickupList(const string& pickupList, const int pickupFloorNum) const {
    ...
    bool capacity = pickupList.size() <= ELEVATOR_CAPACITY;
    
    int max = pickupList[0] - '0';
    ...
    bool Max = false;
    if ((max + 1) <= f.getNumPeople()) {
        Max = true;
    }
    
    int same = 0;
    for (int l = 0; l < pickupList.size() - 1; l++) {
        int direction_0 = f.getPersonByIndex(pickupList[l] - '0').getTargetFloor() - f.getPersonByIndex(pickupList[l] - '0').getCurrentFloor();
        int direction_1 = f.getPersonByIndex(pickupList[l+1] - '0').getTargetFloor() - f.getPersonByIndex(pickupList[l+1] - '0').getCurrentFloor();
        if ((direction_0 * direction_1) < 0) {
            same++;
        }
    }
    if (same==0 && dup==0 && digit==0 && capacity && Max) {
        return true;
    } else {
        return false;
    }
